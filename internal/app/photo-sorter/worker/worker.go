...
func Worker(ctx context.Context, id int, jobs <-chan string, results chan<- error, wg *sync.WaitGroup, cfg *config.Config, logger *logger.Logger, progress *progress.Progress, stats *stats.Stats) {
	defer wg.Done()
	logger.LogInfo(fmt.Sprintf("Worker %d 已啟動", id))
	for {
		select {
		case <-ctx.Done():
			logger.LogInfo(fmt.Sprintf("Worker %d 已停止", id))
			return
		case path, ok := <-jobs:
			if !ok {
				logger.LogInfo(fmt.Sprintf("Worker %d 已完成所有工作", id))
				return
			}
			logger.LogInfo(fmt.Sprintf("Worker %d 正在處理: %s", id, path))
			progress.Update()
			...
			if err != nil {
				...
				logger.LogInfo(fmt.Sprintf("Worker %d 處理成功: %s", id, path))
